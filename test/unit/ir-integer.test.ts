 ...
 */
function checksumOf(build: NodeSpec[], constant: bigint, value: string): string {
  const nodes = [...build];
  nodes.push(
    node(
...
describe("INTEGER_OP_KIND_DIGITS_TO_INTEGER", () => {
  it("reads a bounded run of digits as a decimal integer", () => {
    expect(checksumOf(digitsToInteger(), 1234n, "12345")).toBe("valid");
    expect(checksumOf(digitsToInteger(), 9999n, "12345")).toBe("invalid");
  });

  it("is indeterminate on a non digit, which reports unsupported", () => {
    expect(checksumOf(digitsToInteger(), 1234n, "12A45")).toBe("unsupported");
  });
});
...
describe("INTEGER_OP_KIND_MOD_DIGITS", () => {
  it("takes the remainder digit by digit, past any integer width", () => {
    // 40 digits: far beyond 2^53 and beyond int64, yet exact.
    const long = "1".repeat(40);
    const expected = BigInt(long) % 97n;

    expect(checksumOf(modDigits(97n), expected, long)).toBe("valid");
  });

  it("is indeterminate on an empty or non digit view", () => {
    expect(checksumOf(modDigits(97n), 0n, "")).toBe("unsupported");
    expect(checksumOf(modDigits(97n), 0n, "12X")).toBe("unsupported");
  });
});
...
describe("INTEGER_OP_KIND_WEIGHTED_SUM", () => {
  ...
  it("pairs RIGHT from the last position", () => {
    ...
    expect(checksumOf(build, 13n, "123")).toBe("valid");
  });

  it("cycles the weights over every position", () => {
    ...
    expect(checksumOf(build, 14n, "123")).toBe("valid");
  });

  it("maps letters to base 36 under ALNUM_BASE36", () => {
    // A is 10, B is 11: 10*1 + 11*1 = 21.
    const build = weighted([1n, 1n], WeightAlignment.LEFT, CharMapping.ALNUM_BASE36);

    expect(checksumOf(build, 21n, "AB")).toBe("valid");
  });

  it("takes a value from its index in a custom alphabet", () => {
    ...
    expect(checksumOf(build, 18n, "J")).toBe("valid");
    expect(checksumOf(build, 19n, "J")).toBe("invalid");
  });

  it("is indeterminate on a code point outside the alphabet", () => {
    const build = weighted([1n], WeightAlignment.LEFT, CharMapping.CUSTOM_ALPHABET, "0123456789");

    expect(checksumOf(build, 0n, "I")).toBe("unsupported");
  });

  it("is indeterminate on a code point no weight pairs with", () => {
    ...
    expect(checksumOf(build, 1n, "1X")).toBe("unsupported");
  });
});
...
describe("INTEGER_OP_KIND_MODULO", () => {
  ...
  it("always lands in [0, modulus)", () => {
    expect(checksumOf(build(7n), 123n % 7n, "123")).toBe("valid");
  });
});
...
describe("INTEGER_OP_KIND_COMPLEMENT", () => {
  ...
  it("yields modulus minus the operand", () => {
    expect(checksumOf(build(97n), 97n - 12n, "12")).toBe("valid");
  });

  it("is indeterminate when the operand sits outside [0, modulus]", () => {
    expect(checksumOf(build(50n), 0n, "123")).toBe("unsupported");
  });
});
...
describe("INTEGER_OP_KIND_REMAINDER_MAP", () => {
  ...
  it("indexes the table with the operand", () => {
    expect(checksumOf(build([5n, 6n, 7n]), 7n, "2")).toBe("valid");
  });

  it("is indeterminate outside the table", () => {
    expect(checksumOf(build([5n, 6n]), 0n, "9")).toBe("unsupported");
  });
});
