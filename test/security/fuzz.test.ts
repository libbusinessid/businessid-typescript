...
import { RULES_BUNDLE_BYTES } from "../../src/assets/rules.generated.js";
import { BundleError } from "../../src/domain/errors.js";
import { loadBundle } from "../../src/runtime/load.js";

/**
 * Fuzzing of the decoder and the bundle validator.
 *
 * The contract is narrow and absolute: arbitrary bytes either load into a
 * usable bundle or raise a `BundleError`. Nothing else is acceptable — not a
 * crash, not an unbounded allocation, not a `TypeError` escaping from the
 * decoder, and above all not a bundle that half loads.
 *
